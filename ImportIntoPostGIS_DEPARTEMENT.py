...
"""
***************************************************************************
    ImportIntoPostGIS_DEPARTEMENT.py
    ---------------------
    Date                 : October 2012
...
class ImportIntoPostGIS_DEPARTEMENT(GeoAlgorithm):
    ...
    def processAlgorithm(self, progress):
        ...
        if 'departement' in nom.lower():
            table = "departement"
        else:
            raise GeoAlgorithmExecutionException(self.tr(u'Mauvais fichier (departement seulement): %s' % connection))
        ...
        progress.setText("")
        progress.setText(u"Importation de la table vers PostgreSQL (cela peut prendre un moment)")
        ...
        progress.setText("")
        progress.setInfo(u"Création de la nouvelle colonne")
        
        sql = """BEGIN;"""
        sql +="""ALTER TABLE "{schema}"."departement" ADD COLUMN "dep_dirno" bool );""".format(dep=dep,table=table,schema=schema)
        sql +="""UPDATE "{schema}"."departement" SET "{schema}"."departement"."dep_dirno" = select distinct ST_Intersects("{schema}"."departement"."geom","{schema}"."tampon_dirno"."geometry") from "{schema}"."departement", "{schema}"."tampon_dirno";""".format(dep=dep,table=table,schema=schema)
        sql +="""COMMIT;"""
        sql = sql.replace('\n', ' ')
    ...
    def defineCharacteristics(self):
        self.name = 'Import DEPARTEMENT'
        self.group = 'Database DIRNO'
        self.addParameter(ParameterVector(self.INPUT,
            self.tr(u'departement à importer')))
